...
j5c_Date j5c_Date::internal_addDays(int days) noexcept
{
    ...
    result.setDay(Day);
    /*
        std::cout << "End: " << result.strDate() << " (Expect: "
              << (days == 58 ? "2017-02-28" : days == 59 ? "2017-03-01" : days == 1 ? "2017-03-01" : "2016-12-31") << ")" << std::endl;
    */
    return result;
}
    j5c_Date j5c_Date::internal_subDays(int days) noexcept
    {
        ...
        int remainingDays = days;  // Negative input
        //std::cout << "Start: " << result.strDate() << ", Days to subtract: " << days << std::endl;
        while (remainingDays != 0)
        {
            int daysInThisMonth = result.getDaysInMonth();
            int daysLeftInMonth = result.m_day;  // Days to start of month
            //std::cout << "Loop: Date=" << result.strDate() << " (Expect: Varies), Remaining=" << remainingDays << ", DaysInMonth=" << daysInThisMonth << ", DaysLeft=" << daysLeftInMonth << std::endl;
            if (remainingDays < 0)
            {
                if (-remainingDays >= daysLeftInMonth)
                {
                    ...
                    //std::cout << "Sub: Back to " << result.strDate() << " (Expect: Prev month end), Remaining=" << remainingDays << std::endl;
                }
                else
                {
                    result.m_day += remainingDays;  // Add negative = subtract
                    remainingDays = 0;
                    //std::cout << "Sub Done: " << result.strDate() << " (Expect: " << (days == -1 ? "2016-12-31" : "Correct") << ")" << std::endl;
                }
            }
            else  // Positive days (unlikely, but complete)
            {
                int daysToEnd = daysInThisMonth - result.m_day + 1;
                if (remainingDays >= daysToEnd)
                {
                    result.m_day = daysInThisMonth;
                    ...
                    // std::cout << "Add: Forward to " << result.strDate() << " (Expect: Next step)" << std::endl;
                }
                else
                {
                    result.m_day += remainingDays;
                    remainingDays = 0;
                    // std::cout << "Add Done: " << result.strDate() << " (Expect: Correct)" << std::endl;
                }
            }
        }
        //std::cout << "End: " << result.strDate() << " (Expect: " << (days == -1 ? "2016-12-31" : "Varies") << ")" << std::endl;
        return result;
    }
    ...
    const j5c_Date& j5c_Date::operator--() noexcept {  // Prefix
        *this = internal_subDays(-1);  // Subtract 1 day
        return *this;
    }
